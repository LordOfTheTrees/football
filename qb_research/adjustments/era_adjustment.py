...
def create_era_adjusted_payment_data(force_refresh=False):
    """
    Creates era-adjusted version of payment data with additional columns for injury projections.
    
    Always applies both era adjustment and injury projection to create a complete dataset
    with both _adj (era-adjusted) and _adj_proj (era-adjusted + projected) columns.
    
    Args:
        force_refresh (bool): Recalculate adjustment factors
    
    Returns:
        DataFrame: Payment data with era-adjusted stats and injury-projected stats
    """
    print("\n" + "="*80)
    ...
    # Fill missing seasons (injury/conduct policy violations) before projection
    print("\n" + "="*80)
    print("FILLING MISSING SEASONS")
    print("="*80)
    from qb_research.adjustments.injury_projection import fill_missing_seasons
    adjusted_df = fill_missing_seasons(adjusted_df)
    
    # Always apply injury projection to create projected columns
    print("\n" + "="*80)
    print("APPLYING INJURY PROJECTION")
    print("="*80)
    from qb_research.adjustments.injury_projection import apply_injury_projection
    adjusted_df = apply_injury_projection(adjusted_df)
    
    # Verify both column sets exist
    adj_cols = [col for col in adjusted_df.columns if col.endswith('_adj') and not col.endswith('_adj_proj')]
    proj_cols = [col for col in adjusted_df.columns if col.endswith('_proj')]
    
    print(f"\n" + "="*80)
    print("COLUMN VERIFICATION")
    print("="*80)
    print(f"✓ Era-adjusted columns: {len(adj_cols)} (e.g., {', '.join(adj_cols[:3])}...)")
    print(f"✓ Projected columns: {len(proj_cols)} (e.g., {', '.join(proj_cols[:3])}...)")
    print(f"✓ Both column sets preserved - ready for comparison analysis")
    
    # Save - single file with both _adj and _adj_proj columns
    output_file = 'qb_seasons_payment_labeled_era_adjusted.csv'
    adjusted_df.to_csv(output_file, index=False)
    print(f"\n✓ Saved data to: {output_file}")
    print(f"  Total columns: {len(adjusted_df.columns)}")
    print(f"  Both era-adjusted (_adj) and projected (_adj_proj) columns included")
